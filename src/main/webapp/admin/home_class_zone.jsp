...
<!DOCTYPE html>
	<html>
        <head>
            ...
            <style type="text/css">
                #show-add-homeClassZone-dialog-button {
                    position: fixed;
                    display: block;
                    right: 0;
                    bottom: 0;
                ...
                .save-homeClassZone-dialog-list mdl-list {
                    width: 600px;
                }
                .save-homeClassZone-dialog-list-item {
                    margin: 0px;
                    padding: 0px;
                }
            </style>
            ...
            <script>
                ...
                function searchCheck() {
                    ...
                    $('#admin-search-form').attr('action', 'HomeClassZone_list?keyword=' + keyword);
                }

                $(function () {
                    $("#mdl-navigation__link--" + "HomeClassZone").addClass('mdl-navigation__link--current');
                    $("#content-header-title").text("首页大分类");
                });
	...
	<body id="homeClassZone-body">
		<!-- homeClassZone列表table -->
		<table class="mdl-data-table mdl-js-data-table mdl-data-table--selectable mdl-shadow--2dp">
			...
			<tbody>
				<c:forEach items="${homeClassZoneList }" var="homeClassZone">
					<tr>
						<td>${homeClassZone.id }</td>
						<td>${homeClassZone.name }</td>
						<td>
							<button id="td-menu${homeClassZone.id }" class="mdl-button mdl-js-button mdl-button--icon">
								<i class="material-icons">more_vert</i>
							</button>
							<ul class="mdl-menu mdl-menu--bottom-left mdl-js-menu mdl-js-ripple-effect"
									for="td-menu${homeClassZone.id }">
								<li class="mdl-menu__item" onclick="javascript:showDeleteDialog(${homeClassZone.id });">删除</li>
								<li class="mdl-menu__item" onclick="javascript:showModifyDialog(${homeClassZone.id }, '${homeClassZone.name}', '${homeClassZone.description }');">修改</li>	
							</ul>
						</td>
					</tr>
				</c:forEach>
		...
		<dialog class="save_dialog mdl-dialog">
			<h4 class="mdl-dialog__title save_dialog_title">添加首页推荐大分类</h4>
			<div class="mdl-dialog__content">
				<form id="save-homeClassZone-form" action="HomeClassZone_save" method="post">
					<input id="save-dialog-id" type="hidden" name="id">
					<ul class="save-homeClassZone-dialog-list mdl-list">
						<li class="save-homeClassZone-dialog-list-item mdl-list__item">
							<span class="mdl-list__item-primary-content">
								<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
									<input class="mdl-textfield__input" type="text" id="save-dialog-name"
											name="name">
									<label class="mdl-textfield__label" for="save-dialog-name">大分类名称</label>
								</div>
							</span>
						</li>
						<li class="save-homeClassZone-dialog-list-item mdl-list__item">
							<span class="mdl-list__item-primary-content">
								<div class="mdl-textfield mdl-js-textfield">
									...
									<label class="mdl-textfield__label" for="description">输入推荐大分类简介...</label>
								</div>
							</span>
						</li>
					</ul>
		...
		<!-- 页面右下角添加HomeClassZone按钮 -->
		<button id="show-add-homeClassZone-dialog-button" onclick="javascript:showAddDialog();"
		...
		<script>
			...
			/* 显示确认删除homeClassZone对话框 */
			function showDeleteDialog(homeClassZoneId) {
				
				deleteDialog.querySelector('.agree').addEventListener('click', function() {
					deleteHomeClassZone(homeClassZoneId)
				});
			...
			/* 删除homeClassZone */
			function deleteHomeClassZone(homeClassZoneId){
				$.post("HomeClassZone_delete.action", {homeClassZoneId: homeClassZoneId}, 
					function(result){
			...
			/* 批量删除homeClassZone */
			function deleteHomeClassZones(ids){
				$.post("HomeClassZone_deleteList.action",{ids: ids},function(result){
					if(result.success){
						showMsg("删除成功！");
						setTimeout(refreshPage, 1000);
					}else{
			...
			/* 显示添加homeClassZone对话框 */
			function showAddDialog() {
				$(".save_dialog_title").html("添加首页推荐大分类");
				saveDialog.showModal();
			}
			
			/* 显示修改homeClassZone对话框 */
			function showModifyDialog(id, name, description) {
				$(".save_dialog_title").html("修改首页推荐大分类");
			...
			/* 绑定保存对话框确认按钮事件 */
 			saveDialog.querySelector('.agree').addEventListener('click', function() {
				...
				$("#save-homeClassZone-form").submit();
				resetValue();
				saveDialog.close();
				showMsg("保存成功！");
				setTimeout(refreshPage, 1000);
