...
<body>
	...
	<nav class="navbar navbar-light bg-faded">
		<a class="navbar-brand" href="User_showIndex">
			...
			${homeClassify.homeClassZone.name } ${homeClassify.name }
		</a>
	</nav>
		...
		<div class="mdl-grid">
			<c:forEach items="${productList }" var="product">
				<div class="card mdl-cell mdl-cell--3-col mdl-cell--4-col-tablet mdl-cell--2-col-phone">
					<img class="card-img-top home-item-card-img" src="${product.covers[0] }" alt="${product.name }"
							...
							<% boolean isFavorite = false; %>
									...
									%>
										<li id="md-li-favorite-${product.id }" class="nav-item" onclick="javascript:removeFromFavorite(${product.id });">
											<i id="md-icon-favorite-${product.id }" class="nav-link-icon material-icons home-item-card-action">favorite</i>
										</li>
									<%
										} else {
									%>
										<li id="md-li-favorite-${product.id }" class="nav-item" onclick="javascript:addToFavorite(${product.id });">
											<i id="md-icon-favorite-${product.id }" class="nav-link-icon material-icons home-item-card-action">favorite_border</i>
										</li>
									<%
										}
									 %>
									...
									%>
										<li id="md-li-cart-${product.id }" class="nav-item" onclick="javascript:removeFromCart(${product.id });">
											<i id="md-icon-cart-${product.id }" class="nav-link-icon material-icons home-item-card-action">shopping_cart</i>
										</li>
									<%
										} else {
									%>
										<li id="md-li-cart-${product.id }" class="nav-item" onclick="javascript:addToCart(${product.id });">
											<i id="md-icon-cart-${product.id }" class="nav-link-icon material-icons home-item-card-action">add_shopping_cart</i>
										</li>
									<%
										}
									 %>
		...
		<nav>
			<ul class="pagination justify-content-center">
				<li class="page-item" id="pager-previous" style="margin: 15px;">
					<a class="page-link" style="border-radius: 20px;" href="HomeClassify_show.action?homeClassifyId=${homeClassify.id }&currentPage=${currentPage-1 }">
